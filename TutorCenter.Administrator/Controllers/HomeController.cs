...
public class HomeController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        var studentDtos = await _sender.Send(new GetLearnersQuery());
        _logger.LogDebug("Got classDtos, tutorDtos, studentDtos!");

        var date = GetByTime(DateTime.Now, ByTime.Month);
        var thisMonthCourseDtoCount = classDtos.Value.Count(x => x.CreationTime >= date);
        ...
        var lastMonthCourseDtoCount = classDtos.Value.Count(x => x.CreationTime >= date);
        ...
        return View(
            new DashBoardViewModel
            {
                ...
                ClassTotalValueModel = new TotalValueModel<CourseForListDto>()
                {
                    ...
                    IsIncrease = thisMonthCourseDtoCount > lastMonthCourseDtoCount,
                    IncreasePercentage =
                        Math.Abs(thisMonthCourseDtoCount - lastMonthCourseDtoCount) * 1.0 /
                        lastMonthCourseDtoCount * 100,
                    Time = ByTime.Month
                },
