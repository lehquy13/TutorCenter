    ...
    : IRequestHandler<CustomerRegisterCommand, Result<AuthenticationResult>>
{
    ...
    public async Task<Result<AuthenticationResult>> Handle(CustomerRegisterCommand command,
        CancellationToken cancellationToken)
    {
        ...
        if (await _unitOfWork.SaveChangesAsync(cancellationToken) <= 0) return Result.Fail("Can not register");
        //Create jwt token
